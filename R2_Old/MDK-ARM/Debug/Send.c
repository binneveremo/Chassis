...
void R1ExchangeData_Decode(UART_HandleTypeDef *huart){
	if(huart->Instance == R1_Exchange_Usart.Instance){
		...
		send.R1_Exchange.pos.r = rad2ang(atan2f(send.R1_Exchange.pos.y - site.now.y,send.R1_Exchange.pos.x - site.now.x));
	}
}
void Send_PositionToR1(void){
	send.R1_Exchange.send[0] = 0xAA;
#ifdef Carbon_Car
	float net_x = vision.pos.car_field.x + 80 * cos(ang2rad(site.now.r));
	float net_y = vision.pos.car_field.y + 80 * sin(ang2rad(site.now.r));
#else 
	float net_x = vision.basket.car_vfield.x  + 400;
	float net_y = vision.basket.car_vfield.y  - 375;
#endif
	send.convert.float_data[0] = net_x;
	send.convert.float_data[1] = net_y;
	memcpy(&send.R1_Exchange.send[1],send.convert.uint8_data,8);
	send.R1_Exchange.send[9] = chassis.Flagof.self_lock;
