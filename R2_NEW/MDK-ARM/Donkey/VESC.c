...
bool VESC_SetRPM(float value, unsigned char id) {
	unsigned int eid = (id & 0xff) | ((unsigned int)CAN_PACKET_SET_RPM << 8);
...
bool VESC_SetMultiturnPos(float value, unsigned char id) {
	unsigned int eid = (id & 0xff) | ((unsigned int)CAN_PACKET_SET_POS_MULTITURN << 8);
...
bool VESC_SetPos(float value, unsigned char id) {
	unsigned int eid = (id & 0xff) | ((unsigned int)CAN_PACKET_SET_POS << 8);
